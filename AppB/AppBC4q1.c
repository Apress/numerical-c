/* Montecarlo sphere (whole sphere in 1st quadrant)*/
/*	Calculation of volume using monte carlo */
/*	by counting relative volumes */
/* integrates (x-2)^2 + (y-2)^2 + (z-2)^2 = 2^2 to your specified limits */
...
main()
{
	...
	for (j = 1;j < iterations;j++)
	{
		...
		if (x >= xlower && y >= ylower && z >= zlower)
		{
			totalvol = totalvol + 1; /* This contains the total number of entries */

			if ((pow((y - 2), 2) + pow((x - 2), 2)) + pow((z - 2), 2) < 4)
			{
