/*trapezium -  trapezium method using inverse functions */
...
main()
{
	...
	printf("enter highest inverse power of x (max 6)");
	scanf("%d", &highpower);
	...
	case 6:
		printf("enter coefficient of inverse x power 6(-9 to 9)");
		scanf("%lf", &p6);
	case 5:
		printf("enter coefficient of inverse x power 5(-9 to 9)");
		scanf("%lf", &p5);
	case 4:
	...
	printf("Equation is:- ");
	printf("%lf x**-6+%lf x**-5+%lf x**-4+%lf x**-3 +%lf x**-2 %lf x**-1= %lf\n", p6, p5, p4, p3, p2, p1, p0);
	...
	yarr[1] = p6 * pow(lower, -6) + p5 * pow(lower, -5) + p4 * pow(lower, -4) + p3 * pow(lower, -3) + p2 * pow(lower, -2) + p1 * pow(lower, -1) + p0;
	/* yarr[strips+1] contains the Last side from our trapezium area formula */
	yarr[strips + 1] = p6 * pow(upper, -6) + p5 * pow(upper, -5) + p4 * pow(upper, -4) + (p3*pow(upper, -3)) + (p2*pow(upper, -2)) + p1 * pow(upper, -1) + p0;
	middlevalues = zero;
	...
	for (i = 1;i < strips;i++)
	{
		...
		yarr[i + 1] = p6 * pow(xposn, -6) + p5 * pow(xposn, -5) + p4 * pow(xposn, -4) + (p3*pow(xposn, -3)) + (p2*pow(xposn, -2)) + p1 * pow(xposn, -1) + p0;
